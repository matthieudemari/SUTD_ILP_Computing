...
def is_over(board):
    """
    Check the state of a tic-tac-toe board represented as a 3x3 list of lists.
    
    Returns:
        1 or 2 if that player has won,
        -1 if the board is full with no winner (draw),
        0 if the game is still ongoing.
    """

    # Check first vertical line
    if board[0][0] == board[1][0] == board[2][0] and board[0][0] in [1, 2]:
        return board[0][0]
    
    # Check second vertical line
    if board[0][1] == board[1][1] == board[2][1] and board[0][1] in [1, 2]:
        return board[0][1]
    
    # Check third vertical line
    if board[0][2] == board[1][2] == board[2][2] and board[0][2] in [1, 2]:
        return board[0][2]
    
    # Check first horizontal line
    if board[0][0] == board[0][1] == board[0][2] and board[0][0] in [1, 2]:
        return board[0][0]
    
    # Check second horizontal line
    if board[1][0] == board[1][1] == board[1][2] and board[1][0] in [1, 2]:
        return board[1][0]
    
    # Check third horizontal line
    if board[2][0] == board[2][1] == board[2][2] and board[2][0] in [1, 2]:
        return board[2][0]
    
    # Check first diagonal line
    if board[0][0] == board[1][1] == board[2][2] and board[1][1] in [1, 2]:
        return board[1][1]
    
    # Check second diagonal line
    if board[2][0] == board[1][1] == board[0][2] and board[1][1] in [1, 2]:
        return board[1][1]
    
    # Check for full board with no winner, it is a draw
    if all(board[i][j] in [1, 2] for i in range(3) for j in range(3)):
        return -1
    
    # Otherwise, game still going and no winner
    return 0
...
def check_valid_coordinates(board, x_coord, y_coord):
    ...
    if not is_valid:
        ...
        err_str2 = "(You used x_coord = {} and y_coord = {})".format(x_coord, y_coord)
...
def display_board(board):
    """
    Display a graphical tic tac toe board, for any given valid 3x3 board.
    If board is not a valid board, it will print an error message and not display anything.
    """

    # 1. Check for board type and shape.
    correct_type = False
    try:
        bool1 = isinstance(board, list) and len(board) == 3 and all(isinstance(row, list) and len(row) == 3 for row in board)
        bool2 = all(board[i][j] in [0,1,2] for i in range(3) for j in range(3))
        correct_type = bool1 and bool2
    except:
        correct_type = False
    ...
    if not correct_type:
        ...
        err_str2 = "Your board should be a 3x3 list of lists containing [0,1,2] values only."
